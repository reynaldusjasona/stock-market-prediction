...
function LandingPageEditorPage() {
  ...
  useEffect(()=>{
    adminApi.getLandingPage()
      .then(c => setContent({
        hero:c?.hero|| EMPTY.hero,
        features:c?.features?.length ? c.features : EMPTY.features,
        footer:c?.footer|| EMPTY.footer,
        meta:c?.meta|| EMPTY.meta,
      }))
      .catch(err=> setAlert({msg: err.message || 'Could not load content.', type:'error'}))
  ...
  const handleSave = async (e) => {
    ...
    try{
      ...
      setAlert({msg:'Changes saved and published to the live site.', type:'success'})
    } 
	catch (err) {
    ...
    <div>
      <div className="admin-page-header" style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:'1rem' }}>
        <div>
          ...
          <p className="admin-page-sub">Edit marketing copy shown on the public home page.</p>
        </div>
      ...
      <form onSubmit={handleSave}>
        ...
        <div className="admin-card" style={{ marginBottom:'1rem' }}>
          ...
          <div className="admin-card-body">
            <div className="admin-form-group">
              ...
              <input className="admin-form-input" type="text" value={content.hero.headline}
                onChange={e => setH('headline', e.target.value)} placeholder="Make smarter investment decisions" maxLength={80} required/>
            </div>
            <div className="admin-form-group">
              <label className="admin-form-label">Subheading</label>
              <textarea className="admin-form-textarea" value={content.hero.subline}
                onChange={e => setH('subline', e.target.value)} placeholder="AI-powered predictions…" maxLength={200}/>
            </div>
            <div className="admin-form-group" style={{ marginBottom:0 }}>
              <label className="admin-form-label">CTA Button Label</label>
              <input className="admin-form-input" type="text" value={content.hero.cta_label}
                onChange={e => setH('cta_label', e.target.value)} placeholder="Get Started Free" maxLength={40}/>
            </div>
          </div>
        </div>
              ...
              {content.features.map((f, i) => (
                <div key={i} style={{ background:'var(--bg)', border:'1px solid var(--border)', borderRadius:'8px', padding:'1rem' }}>
                  ...
                  <div className="admin-form-group" style={{ marginBottom:'0.75rem' }}>
                    ...
                    <input className="admin-form-input" type="text" value={f.title}
                      onChange={e => setFeat(i,'title',e.target.value)} placeholder={`Feature ${i+1} title`} maxLength={60}/>
                  </div>
                  <div className="admin-form-group" style={{ marginBottom:0 }}>
                    <label className="admin-form-label">Description</label>
                    <textarea className="admin-form-textarea" value={f.body}
                      onChange={e => setFeat(i,'body',e.target.value)} placeholder="Short description…" style={{ minHeight:'60px' }}/>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="admin-card" style={{ marginBottom:'1rem' }}>
          ...
          <div className="admin-card-body">
            <div className="admin-form-group">
              ...
              <input className="admin-form-input" type="text" value={content.footer.tagline}
                onChange={e => setFoot('tagline',e.target.value)} placeholder="All features are completely free." maxLength={120}/>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem' }}>
              <div className="admin-form-group" style={{ marginBottom:0 }}>
                <label className="admin-form-label">Page Title (SEO)</label>
                <input className="admin-form-input" type="text" value={content.meta.title}
                  onChange={e => setMeta('title',e.target.value)} placeholder="StockWise AI" maxLength={70}/>
              </div>
              <div className="admin-form-group" style={{ marginBottom:0 }}>
                <label className="admin-form-label">Meta Description (SEO)</label>
                <input className="admin-form-input" type="text" value={content.meta.description}
                  onChange={e => setMeta('description',e.target.value)} placeholder="AI-powered stock predictions…" maxLength={160}/>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', gap:'0.75rem' }}>
          <button type="button" className="btn-admin btn-ghost" disabled={saving}
            onClick={() => { setDirty(false); window.location.reload() }}>Discard Changes</button>
          <button type="submit" className="btn-admin btn-primary" disabled={saving}>
            {saving ? <><span className="admin-spinner"/> Saving…</> : 'Save & Publish'}
          </button>
        </div>
      </form>
    </div>
