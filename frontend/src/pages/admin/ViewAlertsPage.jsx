...
import {showToast, debounce} from '../../js/api'
...
function ViewAlertsPage(){
  ...
  useEffect(()=>{
    loadAlerts(); 
	loadSummary()
    timer.current=setInterval(()=>{loadAlerts(); loadSummary()},60000)
        ...
        <div className="admin-card-header" style={{ flexDirection:'column', alignItems:'flex-start', gap:'0.75rem' }}>
          <div style={{ display:'flex', width:'100%', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'0.75rem' }}>
            <div className="admin-search-wrap">
              ...
              <input className="admin-search-input" type="text" placeholder="Search alerts…"
                onChange={e => { setPage(1); dbs(loadAlerts, e.target.value.trim()) }}
                autoComplete="off"/>
            </div>
