...
const avatarColor= s=>{let h=0; 
					   for(const c of(s||''))h=(h*31+c.charCodeAt(0))%AVT.length; 
						return AVT[h] 
					  }
const initials= s=>(s||'?').split(' ').map(w=>w[0]).join('').toUpperCase().slice(0,2)

function ViewUserAccountPage({target, onClose, onEdit, onSuspend, onDelete}){
          ...
          <div className="admin-modal-footer" style={{justifyContent:'space-between'}}>
            ...
            <div style={{display:'flex', gap:'0.75rem'}}>
              <button className="btn-admin btn-warn"
                onClick={()=>{onClose?.(); onSuspend?.({id:user.id||user.user_id, name:user.name||user.email, isSuspended:suspended })}}>
