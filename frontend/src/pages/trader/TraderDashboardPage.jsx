...
import TraderOverviewPage from './TraderOverviewPage'
...
function Icon({ name, size = 15 }) {
  ...
  const icons = {
    ...
    chat:  <svg width={size} height={size} viewBox="0 0 16 16" fill="none"><path d="M2 2h12a1 1 0 011 1v7a1 1 0 01-1 1H5l-3 3V3a1 1 0 011-1z" {...p}/></svg>,
...
function TraderDashboardPage() {
  ...
  useEffect(() => {
    document.title = 'Trader Dashboard — StockWise AI'
    if (locked) return
    traderApi.getSignalsForReview()
      .then(s => setPendingCount((Array.isArray(s) ? s : []).filter(x => !x.endorsement).length))
      .catch(() => {})
  }, [locked])
        ...
        <div className="trader-lock-card">
          <div className={`trader-lock-icon${rejected ? ' rejected' : ''}`}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              <rect x="4" y="10" width="16" height="10" rx="2" stroke={rejected ? '#ff4444' : '#ffd600'} strokeWidth="1.6"/>
              <path d="M8 10V7a4 4 0 018 0v3" stroke={rejected ? '#ff4444' : '#ffd600'} strokeWidth="1.6" strokeLinecap="round"/>
            </svg>
          </div>
          <h1 className="trader-lock-title">{rejected ? 'Application Rejected' : 'Awaiting Admin Approval'}</h1>
          ...
          <a href="/dashboard" className="btn-admin btn-ghost" style={{ textDecoration:'none' }}>← Back to investor view</a>
        </div>
      </div>
    )
  }
    ...
    <div className="trader-shell">
        ...
        <nav className="trader-nav">
          {NAV.map(item => (
            <button key={item.key}
              ...
              {item.label}
              {item.key === 'signals' && pendingCount > 0 && (
                <span className="trader-nav-badge">{pendingCount}</span>
              )}
            </button>
          ))}
        </nav>
      ...
      <div className="trader-main">
        <header className="trader-topbar">
          ...
          <button className="trader-avatar-btn" title="My Account" onClick={() => setShowAccount(true)}>
        ...
        <div className="trader-breadcrumb">
          ...
          <span className="trader-breadcrumb-current">{NAV.find(n => n.key === tab)?.label}</span>
        </div>

        <div className="trader-content">
          {tab === 'overview' && <TraderOverviewPage onNav={setTab}/>}
