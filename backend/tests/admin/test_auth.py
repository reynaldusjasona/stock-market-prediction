...
class TestUC1Login:
    ...
    def test_A_1_WB_request_shape(self):
        ...
        with patch("app.routers.auth.svcLogin", return_value=mock_result), patch(
            ...
            r = client.post(
                "/api/auth/login", json={"email": "admin@test.com", "password": "correct"}
            )
...
class TestUC3ResetPassword:
    def test_A_3_BB_unknown_email(self):
        ...
        with patch("app.routers.auth.changePassword", side_effect=Exception("not found")):
            r = local_client.post(
                "/api/auth/reset-password", json={"old_password": "x", "new_password": "y"}
            )
        assert r.status_code == 500

    def test_A_3_WB_payload_is_email_only(self):
        with patch(
            "app.routers.auth.changePassword", return_value={"message": "ok"}
        ) as mock_change:
            client.post(
                "/api/auth/reset-password", json={"old_password": "old", "new_password": "new"}
            )
