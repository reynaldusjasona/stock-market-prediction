...
class TestUC15ManageAPIs:
    ...
    def test_A_15_FN_add_then_appears_in_list(self):
        with patch(
            "app.routers.admin.createApiSource", return_value={"id": "api4", "name": "New Source"}
        ), patch("app.routers.admin.logActivity", return_value=None), patch(
            "app.routers.admin.getApiSources", return_value=[{"name": "New Source"}]
        ):
            client.post(
                "/api/admin/apis",
                json={"name": "New Source", "base_url": "https://x.com"},
...
class TestUC16ViewAPI:
    ...
    def test_A_16_FN_view_then_edit_flow(self):
        ...
        with patch("app.routers.admin.getApiSourceById", return_value=api), patch(
            "app.routers.admin.updateApiSource", return_value={"name": "Finnhub Updated"}
        ), patch("app.routers.admin.logActivity", return_value=None):
            client.get("/api/admin/apis/api1", headers=ADMIN_HEADERS)
            r = client.patch(
                "/api/admin/apis/api1", json={"name": "Finnhub Updated"}, headers=ADMIN_HEADERS
            )
...
class TestUC17AddAPI:
    def test_A_17_BB_missing_name_rejected(self):
        r = client.post(
            "/api/admin/apis", json={"base_url": "https://x.com"}, headers=ADMIN_HEADERS
        )
    ...
    def test_A_17_WB_correct_field_names(self):
        ...
        with patch("app.routers.admin.createApiSource", return_value=payload) as mock_create, patch(
    ...
    def test_A_17_FN_full_creation_workflow(self):
        with patch(
            "app.routers.admin.createApiSource", return_value={"id": "api5", "name": "New API"}
        ), patch("app.routers.admin.logActivity", return_value=None):
            r = client.post(
                "/api/admin/apis",
                json={"name": "New API", "base_url": "https://new.com"},
                headers=ADMIN_HEADERS,
            )
...
class TestUC18EditAPI:
    def test_A_18_BB_rate_limit_updated(self):
        with patch("app.routers.admin.updateApiSource", return_value={"rate_limit": "120"}), patch(
            "app.routers.admin.logActivity", return_value=None
        ):
            r = client.patch(
                "/api/admin/apis/api1", json={"rate_limit": "120"}, headers=ADMIN_HEADERS
            )
        assert r.json()["rate_limit"] == "120"

    def test_A_18_WB_payload_types_match_schema(self):
        with patch("app.routers.admin.updateApiSource", return_value={}) as mock_upd, patch(
            "app.routers.admin.logActivity", return_value=None
        ):
    ...
    def test_A_18_FN_edit_then_delete_different_source(self):
        with patch("app.routers.admin.updateApiSource", return_value={"rate_limit": "50"}), patch(
            "app.routers.admin.deleteApiSource", return_value=True
        ), patch("app.routers.admin.logActivity", return_value=None):
            r1 = client.patch(
                "/api/admin/apis/api1", json={"rate_limit": "50"}, headers=ADMIN_HEADERS
            )
