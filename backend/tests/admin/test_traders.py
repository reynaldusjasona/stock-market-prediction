...
class TestUC29ViewAllTraders:
    ...
    def test_A_29_FN_approve_moves_between_tabs(self):
        with patch(
            "app.routers.admin.getAllUserAccount",
            return_value=[{"id": "t1", "trader_status": "pending"}],
        ), patch(
            "app.routers.admin.approveTrader", return_value={"trader_status": "approved"}
        ), patch(
...
class TestUC30ViewTraderDetails:
    def test_A_30_BB_auto_verification_result(self):
        with patch(
            "app.routers.admin.verifyLicense",
            return_value={"valid": True, "authority": "CFA Institute"},
        ):
    ...
    def test_A_30_WB_verification_fires_before_approve_enabled(self):
        with patch(
            "app.routers.admin.verifyLicense", return_value={"valid": True}
    ...
    def test_A_30_FN_approved_trader_gains_access(self):
        with patch("app.routers.admin.verifyLicense", return_value={"valid": True}), patch(
            "app.routers.admin.approveTrader", return_value={"trader_status": "approved"}
