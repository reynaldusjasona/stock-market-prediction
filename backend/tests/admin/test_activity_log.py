...
class TestUC27ActivityLog:
    def test_A_27_BB_log_displayed(self):
        with patch("app.routers.admin.getActivityLogs", return_value=[{"action": "login", "admin_name": "Test Admin"}]):
    ...
    def test_A_27_WB_admin_name_joined_not_raw_uuid(self):
        with patch("app.routers.admin.getActivityLogs", return_value=[{"admin_name": "Test Admin", "admin_email": "a@test.com"}]):
            r = client.get("/api/admin/activity-log", headers=ADMIN_HEADERS)
    ...
    def test_A_27_FN_new_action_appears_at_top(self):
        ...
        with patch("app.routers.admin.svcSuspendAccount", return_value={}), \
